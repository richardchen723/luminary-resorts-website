...
export async function POST(
  ...
  try {
    ...
    const body = updateThreadStatusSchema.parse(await request.json())
    const thread = await updateAdminThreadStatus(resolvedParams.id, body.status)
    ...
    if (error?.name === "ZodError") {
      return NextResponse.json(
        { error: "Invalid status payload", details: error.flatten?.() },
        { status: 400 }
      )
    }
